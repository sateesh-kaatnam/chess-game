...
public class Move
{
    ...
    public static ChessPiece[,] boardState = null;
    ...
    //      -1 checkmate
    //      -2 only 2 kings left
    public static int GetPossibleMoves(int[,] boardMatrix, bool forWhite, ref List<Move> moves, ChessPiece[,] pieces)
    {
        int count = 0;
        for (int i = 0; i < 8; i++)
        {
            for (int j = 0; j < 8; j++)
            {
                ...
                if (((forWhite && pieceType > 0) || (!forWhite && pieceType < 0)) && pieces[i,j]!=null)
                {
                    
                    bool[,] possibleMoves = pieces[i,j].getPossibleMoves(pieces);
                    GetPossibleMovesPiece(i, j, pieceType, boardMatrix, ref moves, pieces[i, j],possibleMoves);
                }
            }
        }
    ...
    private static void GetMovesPawn(int i, int j, int k, int[,] boardMatrix, ref List<Move> moves, bool[,] possibleMoves)
    {
        bool forWhite = k > 0;
    ...
    public static void GetPossibleMovesPiece(int i, int j, int pieceType, int[,] boardMatrix, ref List<Move> moves, ChessPiece piece, bool[,] possibleMoves)
    {
        int k = (pieceType > 0) ? 1 : -1;
        if(possibleMoves != null)
        {
            addPossibleMoves(i, j, k, boardMatrix, ref moves, possibleMoves);
        }
        /*switch (k * pieceType)
        {
            // for pawn
            case 1:
                GetMovesPawn(i, j, k, boardMatrix, ref moves, possibleMoves);
                break;
    ...
    /*public static void GetPossibleMovesPiece(int[] square, int pieceType, int[,] boardMatrix, ref List<Move> moves)
    {
        ...
        GetPossibleMovesPiece(square[0], square[1], pieceType, boardMatrix, ref moves, boardState[square[0], square[1]]);
    }*/
    ...
    {
        int temp = BoardManager.ApplyMove(ref boardMatrix, move);

        // if the king is check after applying the move, move is not valid
        if (IsCheck(boardMatrix, forWhite))
        {
            BoardManager.ReverseMove(ref boardMatrix, move, temp);
            return;
        }
        BoardManager.ReverseMove(ref boardMatrix, move, temp);
    ...
    public static void addPossibleMoves(int startI, int startJ, int k, int[,] boardMatrix, ref List<Move> moves, bool[,] possibleMoves)
    {
        bool forWhite = k > 0;
        for(int i = 0; i < 8; i++)
        {
            for(int j = 0; j < 8; j++)
            {
                ...
                {
                    AddMove(new Move(startI, startJ, i, j), boardMatrix, forWhite, ref moves);
                }
            }
        }
    }
